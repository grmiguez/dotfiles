...
forbidden_classes="Polybar Conky Gmrun lattedock plasmashell"
...
generate_window_list() {
	...
	while IFS="[ .\.]" read -r wid ws cname cls host title; do
		...
		# Replacing regular app names for icons
		if [ "${w_name}" = firefox ]; then
			w_name=""
		fi

		if [ "${w_name}" = konsole ]; then
			w_name=""
		fi
		...
		if [ "${w_name}" = Slack ]; then
			w_name=""
		fi

		if [ "${w_name}" = krunner ]; then
			w_name=""
		fi
		...
		if [ "${w_name}" = Spotify ]; then
			w_name=""
		fi

		if [ "${w_name}" = Vncviewer ]; then
			w_name=""
		fi

		if [ "${w_name}" = jetbrains-phpstorm ]; then
			w_name=""
		fi

		if [ "${w_name}" = jetbrains-idea-ce ]; then
			w_name=""
		fi

		if [ "${w_name}" = systemsettings ]; then
			w_name=""
		fi

		if [ "${w_name}" = DBeaver ]; then
			w_name=""
		fi
	
		if [ "${w_name}" = Postman ]; then
			w_name=""
		fi

		if [ "${w_name}" = dolphin ]; then
			w_name=""
		fi
